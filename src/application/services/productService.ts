...
export class ProductService {
    ...
    static async createProduct(name: string, price: string, amount:string): Promise<Product> {
        const product = new Product(null, name, price, amount);
        return  await ProductService.productRepository.save(product);
    }

    static async getProductById(id: string): Promise<Product| null>{
        return await ProductService.productRepository.findbyID(id);
    }
    ...
    static async updateProduct(id: string, name: string, price: string,amount:string): Promise<Product> {
        const product = new Product(id, name, price, amount);
